...
<main class="container mx-auto px-4 py-8">
    <section>
        ...
        <div id="loading" class="flex flex-col items-center justify-center min-h-[60vh]">
            ...
            <p class="text-[var(--light-text)] animate-pulse">Loading series...</p>
        </div>
        ...
        <div id="no-results" class="hidden flex-col items-center justify-center min-h-[60vh] text-center">
            ...
            <h2 class="text-2xl font-bold">No series found</h2>
            <p class="text-[var(--light-text)] mt-2">Could not retrieve any series at this time.</p>
        </div>
        ...
        <div id="error" class="hidden flex-col items-center justify-center min-h-[60vh] text-center">
            ...
            <p class="text-[var(--light-text)] mt-2">Failed to load series. Please try again later.</p>
            <button onclick="fetchSeriesList()" class="mt-6 bg-[var(--dark-slate)] hover:bg-[var(--border-color)] px-6 py-2 rounded-lg transition">Retry</button>
        </div>
...
<script>
    const SERIES_API_URL = '/api/anime-world-india/v1/series.php';
    let currentPage = 1;
    let totalPages = 1;
    ...
    async function fetchSeriesList() {
        ...
        try {
            const response = await fetch(`${SERIES_API_URL}?p=${currentPage}`);
            const data = await response.json();

            loadingEl.classList.add('hidden');

            if (data.success && data.series && data.series.length > 0) {
                renderGrid('series-grid', data.series, 'series');
    ...
    document.getElementById('prev-page').addEventListener('click', () => {
        if (currentPage > 1) {
            window.location.href = `allseries.php?p=${currentPage - 1}`;
        }
    });
    ...
    document.addEventListener('DOMContentLoaded', fetchSeriesList);
